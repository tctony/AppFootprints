...
config = ConfigParser.RawConfigParser({ 'objc_map_file_dir': '.' })
config.read(script_dir+'appfootprint.conf')
objc_map_file_dir = os.path.abspath(script_dir+config.get('AppFootprint', 'objc_map_file_dir'))+'/'
print objc_map_file_dir
...
actions = []
signature = md5.new()
pattern = re.compile('LogFootprint\(@\"(.*?)\",')
for s in sources:
    with open(s) as fp:
        ln = 0
        for line in fp:
...
fp_h = open(objc_map_file_dir+'FootprintMap.h', 'w');
...
fp_m = open(objc_map_file_dir+'FootprintMap.m', 'w')
...
fp_py = open(script_dir+'FootprintMap.py', 'w');
